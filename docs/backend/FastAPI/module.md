...
:::tip
在Web应用程序中，路由分组和模块化是一种组织和管理路由的常见方法，可以使代码更具可读性、可维护性和可扩展性。

路由分组是将相关的路由路径和处理程序组合在一起的方式。通过将路由分组到不同的组，可以更清晰地组织和区分不同功能或模块的路由。这样可以使代码结构更清晰，更易于理解和维护。常见的路由分组方式包括根据功能、模块、资源类型等进行分组。

模块化是将应用程序划分为独立的模块或组件的方法。每个模块可以包含自己的路由和处理程序，处理特定的功能或业务逻辑。模块化的设计使得应用程序可以更好地分解为可管理的部分，每个部分负责特定的任务。这样可以提高代码的可重用性和可扩展性，并允许团队并行开发不同模块。
:::
...
- 定义公共函数,在多个路径操作中调用
- 在同一个路径操作装饰器中定义多个路径,共用参数
- 设置路径参数的默认值,让其他路径操作自动继承
- 路径操作函数重载,根据请求方法选择正确执行的操作
