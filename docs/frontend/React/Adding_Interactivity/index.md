...
```jsx
export default function App2_0_1() {
  return (
    <Toolbar
...
export default function Gallery2_0_2() {
  const [index, setIndex] = useState(0);
  const [showMore, setShowMore] = useState(false);
...
<TabItem value="Data.js" label="Data.js">
...
[作为快照的状态](# State如同一张快照) 解释了为什么会出现这种情况。设置状态会请求一个新的重新渲染，但不会在已运行的代码中更改它。所以在你调用 `setScore(score + 1)` 后，`score` 仍然是 `0` 。
...
<Tabs>
<TabItems value="App.js" Label="App.js">

```jsx
import { useState } from 'react';

export default function Counter2_0_5() {
  const [score, setScore] = useState(0);
